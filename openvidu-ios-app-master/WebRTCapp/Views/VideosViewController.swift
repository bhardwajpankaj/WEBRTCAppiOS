...
//    @IBOutlet weak var remoteVideoView2: UIView!
    ...
    var bsServer = "https://nishulk.com:443" //"https://172.105.40.87:12123"//
    var cookie: String = ""
    var token: String = ""
...
//        remoteViews?.append(remoteVideoView2)
    ...
    @IBAction func backAction(_ sender: UIButton) {
        ...
        self.removeSession(cookie: self.cookie, token: self.token)
    }
    ...
    func removeSession(cookie:String,token:String)
    {
        let headers = [
        ...
        let url = URL(string: "https://172.105.40.87:12123/api-sessions/remove-user")!
        var request = URLRequest(url: url)
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        
        request.httpMethod = "POST"
        request.allHTTPHeaderFields = headers
        request.httpBody = "{\"sessionName\": \"\(self.sessionName)\",\"token\": \"\(token)\"}".data(using: .utf8)
    ...
    func startNew() {
            let url = URL(string: "https://172.105.40.87:12123/api-login/login")!
            var request = URLRequest(url: url)
            request.setValue("application/json", forHTTPHeaderField: "Content-Type")
            request.httpMethod = "POST"
            let json = "{\"user\": \"publisher1\",\"pass\": \"pass\"}"
            ...
            let task = session.dataTask(with: request) { data, response, error in
               ...
               let realResponse = response as? HTTPURLResponse
                ...
                let cookies = HTTPCookie.cookies(withResponseHeaderFields: fields ?? [:], for: response?.url ?? URL(fileURLWithPath: ""))
                ...
                let url = URL(string: "https://172.105.40.87:12123/api-sessions/get-token")!
                var request = URLRequest(url: url)
                request.setValue("application/json", forHTTPHeaderField: "Content-Type")
                
                request.httpMethod = "POST"
                request.allHTTPHeaderFields = headers
                request.httpBody = "{\"sessionName\": \"\(self.sessionName)\"}".data(using: .utf8)
                ...
                let dataTask = session.dataTask(with: request) { data, response, error in
                  if (error != nil) {
                    ...
                    do {
                        ...
                        print(jsonArray)
                        self.token = jsonArray?["0"] as? String ?? ""
                        if(self.token.count > 0){
                            self.createSocket(token: self.token)
                        ...
                        {
                            self.createSocket(token: "wss://nishulk.com?sessionId=ses_RIZzpR8zzJ&token=tok_Ob0tiKdVDWaIxgOs&role=PUBLISHER&version=2.14.0&coturnIp=172.105.40.87&turnUsername=C4WHGK&turnCredential=id0ksg")
    ...
    func start() {
		if self.url == bsServer + "/openvidu" {
			let url = URL(string: bsServer + "/api/sessions")!
            var request = URLRequest(url: url)
            request.setValue("application/json; charset=utf-8", forHTTPHeaderField: "Content-Type")
            request.addValue("Basic " + ("OPENVIDUAPP:SECRET_CODE_LINODE".data(using: String.Encoding.utf8)?.base64EncodedString() ?? ""), forHTTPHeaderField: "Authorization")
            request.httpMethod = "POST"
            let json = "{\"customSessionId\": \"\(self.sessionName)\"}"
            request.httpBody = json.data(using: .utf8)
            var responseString = ""
            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                // Get Token
                let url = URL(string: self.bsServer + "/api/tokens")!
                var request = URLRequest(url: url)
                request.setValue("application/json; charset=utf-8", forHTTPHeaderField: "Content-Type")
                request.addValue("Basic " + ("OPENVIDUAPP:SECRET_CODE_LINODE".data(using: String.Encoding.utf8)?.base64EncodedString() ?? ""), forHTTPHeaderField: "Authorization")
                request.httpMethod = "POST"
                ...
                let task = URLSession.shared.dataTask(with: request) { data, response, error in
                    ...
                    do {
                        ...
                        if jsonArray?["token"] != nil {
							...
							token = "wss://nishulk.com?sessionId=SessionD&token=tok_Aw8k58aczuKXAMSc&role=PUBLISHER&version=2.14.0&coturnIp=172.105.40.87&turnUsername=3OHQVZ&turnCredential=j6f2g4"
                        }
                    } catch let error as NSError {
                        print(error)
                    }
    ...
    func createSocket(token: String) {
        self.socket = WebSocketListener(url: "wss://nishulk.com:443/openvidu", sessionName: self.sessionName, participantName: self.participantName, peersManager: self.peersManager!, token: token, views: remoteViews!, names: remoteNames!)
...
extension VideosViewController: URLSessionDelegate {
