...
class PeersManager: NSObject,RTCPeerConnectionDelegate {
...
//    var connectionConstraints: RTCMediaConstraints?
    ...
    func createRemotePeerConnection(remoteParticipant: RemoteParticipant) {
        ...
        let videoEncoderFactory = RTCDefaultVideoEncoderFactory()
        let videoDecoderFactory = RTCDefaultVideoDecoderFactory()
        let peerConnectionFactory2 = RTCPeerConnectionFactory(encoderFactory: videoEncoderFactory, decoderFactory: videoDecoderFactory)
        ...
        remotePeer = peerConnectionFactory2.peerConnection(with: config, constraints: sdpConstraints, delegate: self)
